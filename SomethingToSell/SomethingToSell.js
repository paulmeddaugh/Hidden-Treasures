...
/*loads head items into an array of objects and determines how many duplicates they have
var item[] = {numOfDuplicates: 0};
var headItems = document.getElementsByClassName('HeadItem');
for (i = 0; i < headItems.length; i++) {
	item[i].numOfDuplicates = 0;
} */
...
function displayUser() {
	...
	var xhr = new XMLHttpRequest();
				...
				var rowCount = jsonData.length;
				 ...
				 if (rowCount > 0) {
					...
					for (var i = 0; i < rowCount; i++) {
						...
						tstring += "<input type=\"number\" value=\"0\" class=\"QuantityOnMarket\" min=\"0\" align=center";
			 ...
			 xhr.open("GET", "../AccountInformation/displayIown.php?user=" + user, false);
			 xhr.send();
	...
	xh.open("GET", "onSaleI.php?user=" + user, false);
	xh.send();
	
	for (i = 0; i < table.rows.length - 1; i++) {
		itemIds[i + 1] = table.rows[i + 1].dataset.iid;
		console.log(itemIds[i + 1]);
	}
...
function SubmitData (event) {
	...
	for(var q = 0; q < itemIds.length-1; q++){
		...
		if(countEmpty == num / 2){
			...
			var xhr = new XMLHttpRequest();
			xhr.open("POST", "SellItems.php", true);
