...
function displayUser() {
	...
	document.getElementById("bal").innerHTML = "Balance: " + sessionStorage.getItem('balance');
	...
	xhr.onreadystatechange = function () {
		if (xhr.readyState == 4 && xhr.status == 200) {
			...
			if (rowCount > 0) {
				...
				var tstring = ["<table>",
					"<tr><th> Seller </th><th> Quantity Available </th><th> Item Name </th><th> Description </th>",
					"<th> Price </th><th> Quantity To Buy </th></tr>"];
				for (var i = 0; i < rowCount; i++) {
					var rowdata = jsonData[i];
					if (rowdata.sellquan > 0) {
						tstring.push("<tr><td>"+rowdata.nameofUser+"</td>",
							"<td class=q>"+rowdata.sellquan+"</td>",
							"<td>"+rowdata.Iname+"</td>",
							"<td>"+rowdata.descrip+"</td>",
...
function chkQuantity(quan, rownum) {
	...
	function chkqty(){
		...
		if(countEmpty == inputs.length-1){
			...
			var xhr = new XMLHttpRequest();
			xhr.open("POST", "buyItems.php", true);
			...
			xhr.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
			 ...
			 xhr.onreadystatechange = function () {
				if (xhr.readyState == 4 && xhr.status == 200) {
					...
					window.open('../AccountInformation/AccountInformation.html', '_self');
					}
